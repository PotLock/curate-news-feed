...
function OverviewTab({ session }: { session: any }) {
  ...
  const articlesReadToday = historyArticles.filter(article => 
    new Date(article.readAt).toDateString() === today
  ).length;
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col lg:flex-row gap-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="w-full lg:w-[357px] h-48 rounded-xl bg-gray-200 animate-pulse p-6">
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="w-full lg:w-[357px] h-48 rounded-xl bg-gradient-to-br from-[#2B7FFF] to-[#155DFC] shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1),0_8px_10px_-6px_rgba(0,0,0,0.1)] p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex flex-col items-end">
            <p className="text-white text-right font-inter text-[30px] font-bold leading-9">{totalArticlesRead}</p>
      ...
      <div className="w-full lg:w-[357px] h-48 rounded-xl bg-gradient-to-br from-[#AD46FF] to-[#9810FA] shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1),0_8px_10px_-6px_rgba(0,0,0,0.1)] p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex flex-col items-end">
            <p className="text-white text-right font-inter text-[30px] font-bold leading-9">{articlesReadToday}/3</p>
            <p className="text-[#DBEAFE] text-right font-inter text-base font-normal leading-6">Read Today</p>
          </div>
        </div>
      </div>
    </div>
